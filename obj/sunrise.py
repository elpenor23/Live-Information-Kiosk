...
class sun:
    ...
    def sunrise(self,when=None):
        ...
        if present)
        """
        if when is None : when = datetime.now(tz = self.to_timezone)
    ...
    def isDusk(self, when=None):
        ...
        if (thirty_min_before_sunset <= when <= thirty_min_after_sunset):
           return True
        else:
            return False

    def isDawn(self, when=None):
        if when is None : when = datetime.now(tz = self.to_timezone)
        ...
        if (thirty_min_before_sunrise <= when <= thirty_min_after_sunrise):
           return True
        else:
            return False

    def isDay(self, when=None):
        if when is None : when = datetime.now(tz = self.to_timezone)
        sunrise = self.sunrise(when)
        sunrise_full_date = datetime.strptime(when.strftime("%m/%d/%Y ") + sunrise.strftime("%H:%M:%S"), "%m/%d/%Y %H:%M:%S")
        sunrise_timezone = sunrise_full_date.replace(tzinfo = self.to_timezone)
        
        if (sunrise_timezone >= when <= sunset):
           return True
        else:
            return False
    
    def isNight(self, when=None):
        if when is None : when = datetime.now(tz = self.to_timezone)
        sunset = self.sunset(when)
        sunset_full_date = datetime.strptime(when.strftime("%m/%d/%Y ") + sunset.strftime("%H:%M:%S"), "%m/%d/%Y %H:%M:%S")
        sunset_timezone = sunset_full_date.replace(tzinfo = self.to_timezone)

        if (when >= sunset_timezone):
           return True
        else:
            return False

    def timeOfDay(self, when=None):
        if when is None : when = datetime.now(tz = self.to_timezone)
        
        if self.isDawn(when):
            return "dawn"
        elif self.isDusk(when):
            return "dusk"
        elif self.isDay(when):
            return "day"
        elif self.isNight(when):
            return "night"
        else:
            return "unknown"
